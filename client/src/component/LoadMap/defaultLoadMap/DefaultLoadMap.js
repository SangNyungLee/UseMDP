...
// 서타일
import { _Container, _SidebarCol, _RenderComponentCol, _RenderComponent, _Navbar, _Nav, _NavItem, _Row, _Button } from '../../../constant/css/styledComponents/__DefaultLoadMap';
                ...
                <_Row className="justify-content-end">
                    <aside className="sidebar">
                        <nav className="menu">
                            ...
                            <a className={`menu__item ${menuNumber === 1 ? 'menu__item--active' : ''}`} onClick={(e) => handleNumber(e, 1)}>
                                <i className="menu__icon fa fa-home"></i>
                                <span className="menu__text">시작</span>
                            </a>
                            <a className={`menu__item ${menuNumber === 2 ? 'menu__item--active' : ''}`} onClick={(e) => handleNumber(e, 2)}>
                                <i className="menu__icon fa fa-envelope"></i>
                                <span className="menu__text">인기</span>
                            </a>
                            <a className={`menu__item ${menuNumber === 3 ? 'menu__item--active' : ''}`} onClick={(e) => handleNumber(e, 3)}>
                                <i className="menu__icon fa fa-list"></i>
                                <span className="menu__text">플래너</span>
                            </a>
                            <a className={`menu__item ${menuNumber === 4 ? 'menu__item--active' : ''}`} onClick={(e) => handleNumber(e, 4)}>
                                <i className="menu__icon fa fa-calendar"></i>
                                <span className="menu__text">캘린더</span>
                            </a>
                            <a className={`menu__item ${menuNumber === 5 ? 'menu__item--active' : ''}`} onClick={(e) => handleNumber(e, 5)}>
