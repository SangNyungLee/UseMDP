...
export default function CardStatusSelectList({ cardStatusArr, target, index, plannerId, setIsVisible }) {
            ...
            {isClick ? (
                <_CardContainer>
                    {cardStatusArr.map((card) => (
                        <CardSelectList key={card.cardId} card={card} target={target} plannerId={plannerId} index={index} setIsVisible={setIsVisible} />
                    ))}
                </_CardContainer>
            ) : (
                <></>
