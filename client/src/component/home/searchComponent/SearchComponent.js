...
const nullOptions = [{ value: "null", label: "해당하는 결과가 없습니다" }];
...
export default function SearchComponent() {
  ...
  const handleSearch = (e) => {
    ...
    if (option.value === "stack") {
      setFilteredDatas(
        datas.filter((item) =>
          selectTag.some((tag) => item.taglist.includes(tag.value))
        )
      );
    } else if (option.value === "author") {
      setFilteredDatas(
        datas.filter((item) =>
          selectTag.some((author) => item.creator.includes(author.value))
        )
      );
    } else if (option.value === "title") {
      setFilteredDatas(
        datas.filter((item) =>
          selectTag.some((title) => item.title.includes(title.value))
        )
      );
    }
  };
  return (
    <_ComponentContainer>
      ...
      {option.value == "stack" && (
        <>
          <_ComponentTitle style={{ marginTop: "30px", marginBottom: "10px" }}>
            선택한 태그
          </_ComponentTitle>{" "}
