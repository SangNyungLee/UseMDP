...
export default function DroppableComponent(props) {
  ...
  const addCard = async (e) => {
    e.stopPropagation();
    const cardStatus =
      cardStatusIndex === 0 ? "TODO" : cardStatusIndex === 1 ? "DOING" : "DONE";
    ...
    const result = await axios.post(
      "http://localhost:8080/api/postCard",
      card,
      { withCredentials: true }
    );
    dispatch(
        ...
        return (
          <>
            <div {...droppableComponentRegister(provided, snapshot)}>
              <CardListHeader
                index={provided.droppableProps["data-rbd-droppable-id"]}
              ></CardListHeader>
              {cardList.map((card, id) => (
                ...
                >
                  {(provided, snapshot) => (
                    <div
                      {...draggableComponentRegister(provided, snapshot, id)}
                    >
                      <QuoteCard
                        card={card}
